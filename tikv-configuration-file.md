...
+ 指定当检测到非法的 `max-ts` 更新请求时，TiKV 的处理方式。如果某个读写请求使用的 TS 超过了 TiKV 缓存的 PD TSO + [`max-drift`](#max-drift-从-v900-版本开始引入)，TiKV 会将其视为非法的 `max-ts` 更新请求。非法的 `max-ts` 更新请求可能破坏 TiDB 集群的线性一致性和事务并发控制语义。
+ 可选值：
...
+ 默认值：256MiB。在 v8.5.4 和 v9.0.0 之前，默认值为 `128MiB`。
+ 最小值：0
+ 单位：B|KiB|MiB|GiB
...
TiKV TopSQL 维度统计上报的对外配置项。

### `enable-network-io-collection` <span class="version-mark">从 v8.5.6 版本开始引入</span>

+ 是否额外采集并上报网络流量与逻辑 IO。
  + `false`：仅按 `sql`, `table`, `db` 维度上报 CPU time（以及必要的基础字段），不生成 `region` 维度上报数据；
  + `true`：除 CPU time 外，上报内容还包含 network/logical IO，并会额外生成并上报 region 维度记录。同时也会改变 topN 的挑选逻辑（不再仅按 CPU time）。
+ 默认值：`false`